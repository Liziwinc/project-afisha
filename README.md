...
Кратко про проект:
- SPA афиша кинотеатра с выбором сеанса, мест и оформлением заказа.
- Реализована модель с бизнес-логикой (`AppState`), контроллеры и экранные компоненты.
- Построена модульная архитектура на базе собственного MVC-подхода и компонентов.
- Есть сохранение состояния корзины и данных пользователя в `localStorage`.

Что особенного:
- Чистый код, типизация на TypeScript (`TS`), строгая структура папок и переиспользуемые компоненты.
- Логика модальных экранов вынесена в абстрактный `ModalScreen` с наследованием.
- Взаимодействие с API через отдельный `FilmApi` и класс `Api` (работа с `fetch`/`XHR`).
- Адаптивный интерфейс с `SASS`/`SCSS`, `CSS`, семантический HTML и кроссбраузерность.
- Настроена сборка через `Webpack` и npm-скрипты (`npm run build`, `npm run start`).

Как запустить проект:
1. `npm install` (или `yarn`).
2. `npm run build` (или `yarn build`).
3. `npm run start` (или `yarn start`).
4. Открыть `http://localhost:8080` (или адрес по конфигурации dev-server).

### Статус
- Работоспособный фронтенд с боевым кейсом: выбор фильма, сеанса, мест, оформление и подтверждение заказа.
- Поддерживает шаблоны компонент, состояние, классы, наследование и OOP-подход.
